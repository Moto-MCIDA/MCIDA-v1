...
$booleen = $db->query("SELECT count(*) FROM ligne WHERE user = '$user_id' AND link ='message tous' AND id_link='0'"); // on compte combien de balade on une date >= a la date du jour 
...
if($bool >= 1){
	...
	$sql = "UPDATE `ligne` SET date_ligne='$date' WHERE user = '$user_id' AND link = 'message tous' AND id_link='0'";
}else{
	$sql = "INSERT INTO `ligne` (user, link, id_link, date_ligne) VALUES ('$user_id', 'message tous', '0', '$date')";
}
...
<!-- -------------------------------- Affichage des messages -------------------------------- -->	
	<div id="message">
		<div id="afficheMsg">
			<div>
				<div id='nom_bar'><h1 id = 'nom'>Message tous les membres</h1></div>
				<?php 
					affiche_message_tous(recup_message_tous($db),$db);
				?>
			</div>
		</div>
<!-- -------------------------------- Ecrire un message -------------------------------- -->
